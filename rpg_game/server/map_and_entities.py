...
class Wolf_cave(Game_map):
	def __init__(self)->None:
		super().__init__('caverna dos lobos','A caverna é um lugar escuro, você ouve goteiras por todo o local e um forte cheiro de carne podre, vinda das vidas que, precocemente foram abatidas enquanto se debatiam em dor.',move_possibilities=(("Floresta Negra")))
...
class Witch_caves(Game_map):
	def __init__(self)->None:
		super().__init__('caverna dos bruxos','A caverna se torna cada vez mais escura, nela você também sente um pequeno resquício de intenção assassina que cobria todo o local, causando calafrios até mesmo no mais corajoso guerreiro que enfrentava a morte diariamente.',1,2,move_possibilities=(("ruina do golens")))
...
class Rotational_forge(Game_map):
	def __init__(self)->None:
		super().__init__('forja rotacional', 'Ao visualizar uma pequena série de motores, você percebe pequenos raios de luz, mostrando que você está próximos da superfície, porém como não existe almoço grátis, você encontra grandes circunferências presas em algo que parecia ser um barco, estas circunferências soltam grossos rolos que possuíam pequenas fios de vidro.',4,2,move_possibilities=(('Templo Sagrado')))
...
class Character:
	...
	def show_status(self,equips:list[str])->str:
		...
		for i in equips:
			equipments_names+='\t'+i+'\n'
		return 'nome: %s\nvida: %.2f/%.2f\ndescrição do equipamento utilizado:%s\nequipamentos:\n'%(self.name,self.life,self.MAX_LIFE,self.used_equipment.show_description(),equipments_names)
...
class Elfo(Enemy):
	def __init__(self) -> None:
		super().__init__("Elfo",00,items.Arco_recurvo,"Um elfo arqueiro que usa apenas de sua habilidade para matar aqueles a sua volta utilizando um arco recurvo")
...
class Mazzutti(Enemy):
	def __init__(self)->None:
		super().__init__('Mazzutti',150,items.Mosquete(),'um carecão lindo pelado com o mosquetão de fora vindo te pegar')
